...
const emailSchema = z.string().trim().email({ message: "Correo no válido" }).max(255);
const passwordSchema = z
  .string()
  .min(6, { message: "Mínimo 6 caracteres" })
  .max(72, { message: "Máximo 72 caracteres" });
const nameSchema = z.string().trim().min(1, { message: "Requerido" }).max(100);
...
function LoginPage() {
    ...
    try {
      ...
      if (mode === "signup") {
        ...
        if (!nameParsed.success) {
          toast.error("Tu nombre completo es requerido");
          return;
        }
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    ...
    try {
      ...
      // If redirected, the browser will navigate. Otherwise tokens are set.
    } catch {
  ...
  const handleResetPassword = async () => {
    ...
    if (!parsed.success) {
      toast.error("Escribe tu correo arriba para enviar el enlace");
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-background px-5 py-10 flex flex-col items-center">
      <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {mode === "signup" && (
              <>
                <div className="space-y-2">
                  <label className="text-display-sm block">Nombre completo</label>
                ...
                <div className="space-y-2">
                  <label className="text-display-sm block">Departamento (opcional)</label>
                  <NeonInput
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-display-sm">Contraseña</label>
                {mode === "login" && (
                  <button
                    type="button"
        ...
        <Link
          to="/"
          className="block text-center text-text-tertiary text-xs mt-6 hover:text-foreground"
        >
          ← Volver al inicio
        </Link>
      </div>
